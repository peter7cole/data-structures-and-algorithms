  ...
  arr.forEach(phoneNumber => {
    ...
    for (let i = 0; i < phoneNumber.length; i++) {
      if (phoneNumber[i] !== '(' &&
          ...
          phoneNumber[i] !== '-' &&
          phoneNumber[i] !== ' ') {
        eachNumArr.push(phoneNumber[i]);
      }
    }
    resultArr.push(eachNumArr.join(''));
  });
  return resultArr;
};
...
const onlyOddChars = (str) => {
  ...
  for (let i = 0; i < str.length; i++) {
    i % 2 !== 0 ? resultArr.push(str[i]) : undefined;
  }
...
const findAnything = (arr, target) => {
  const resultArr = [];
  arr.forEach(str => {
    str.includes(target) ? resultArr.push(str) : undefined;
  });
  return resultArr;
};
...
const unenrollBrook = (arr) => {
  ...
  arr.forEach(people => {
    ...
    for(let i = 0; i < people.length; i++) {
      if(!people[i].includes('Brook')){
        peopleArr.push(people[i]);
      }
    }
    containerArr.push(peopleArr);
  });
  return containerArr;
};
